...
Page({
    ...
    wx.showActionSheet({
      ...
      success:function(res){
        if(res.tapIndex===0){
          ...
          wx.request({
              url: 'http://192.168.1.213/api/index.php?c=eweivideo&a=Usersq&op=deletereply&uniacid=' + getApp().globalData.acid+'&bid=' + id + "&openid=" + getApp().globalData.openid,
            success:function(res){
  ...
  more:function(){
    ...
    if(page<=max){
      ...
      wx.request({
          url: 'http://192.168.1.213/api/index.php?c=eweivideo&a=Usersq&op=getreplys&uniacid=' + getApp().globalData.acid+'&openid=' + getApp().globalData.openid+'&page=' + page,
        success: function (res) {
          THIS.setData({
            hidden:true
          })
  ...
  onReady: function () {
    ...
    //初次获取帖子信息
    wx.request({
        url: 'http://192.168.1.213/api/index.php?c=eweivideo&a=Usersq&op=getreplys&uniacid=' + getApp().globalData.acid+'&openid=' + getApp().globalData.openid+'&page=1',
      success:function(res){
        THIS.setData({
          replys: res.data.dat.list,
        })
